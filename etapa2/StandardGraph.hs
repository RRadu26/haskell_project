...
{-
    data StandardGraph a = Graf
        { nodes :: S.Set a
        , edges :: S.Set (a, a)
        } | GraphNil deriving Show
...
{-
    *** TODO ***
    ...
    fromComponents ns es = Graf (S.fromList ns) (S.fromList es)
...
outNeighbors node graph = S.fromList $ map snd $ filter (\m -> fst m == node) $ S.toList (edges graph)
...
inNeighbors node graph = S.fromList  $ map fst $ filter(\m -> snd m == node) $ S.toList (edges graph)
...
removeNode node graph = fromComponents (S.toList (S.delete node $ nodes graph)) $ filter(\m -> fst m /= node && snd m /= node) $ S.toList (edges graph)
...
splitNode old news graph =
    let ng = removeNode old graph
        nn = S.toList (nodes ng) ++ news
        newedges = foldl (\acc m -> (map (\x-> (x, m)) news) ++ acc) [] (S.toList (outNeighbors old graph)) ++ foldl (\acc m -> (map (\x-> (m, x)) news) ++ acc) [] (S.toList (inNeighbors old graph))
    in fromComponents nn ((S.toList (edges ng)) ++ newedges)
...
mergeNodes prop node graph = 
    let elimn = filter (\x -> prop x) (S.toList (nodes graph)) 
    in foldl (\graf x -> splitNode x [node] graf) graph elimn