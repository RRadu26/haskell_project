...
-}
searchh :: Ord a => ([a] -> [a] -> [a]) -> Graph a -> S.Set a -> [a] -> [a] -> [a]
searchh f graph visited stack acc = if (length stack) == 0 then acc 
    else if S.member (head stack) visited 
        then searchh f graph visited (tail stack) acc 
            else searchh f graph (S.insert (head stack) visited) (f stack (S.toList (outNeighbors (head stack) graph))) (acc ++ [(head stack)])
search :: Ord a
       => ([a] -> [a] -> [a])  -- funcția de îmbinare a listelor de noduri
       -> a                    -- nodul de pornire
       -> Graph a              -- graful
       -> [a]                  -- lista obținută în urma parcurgerii
search f node graph = searchh f graph S.empty [node] [] 
...
bfs node graph= search (\x y -> (tail x) ++ y) node graph
...
dfs node graph = search (\x y -> y ++ (tail x)) node graph
...
countIntermediate from to graph = if (elem to (bfs from graph)) then Just ((length (fst (span (/= to) (bfs from graph)))-1), (length (fst (span (/= to) (dfs from graph))) -1)) else Nothing
